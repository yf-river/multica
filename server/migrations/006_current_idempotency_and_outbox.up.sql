...
CREATE TABLE public.resource_create_request (
    ...
    CONSTRAINT resource_create_request_resource_type_check CHECK ((resource_type = ANY (ARRAY['workspace'::text, 'workspace_member'::text, 'project'::text, 'squad'::text, 'agent'::text, 'skill'::text, 'attachment'::text, 'quick_create'::text, 'issue'::text, 'comment'::text, 'autopilot_trigger'::text, 'issue_rerun'::text, 'runtime_profile'::text, 'label'::text, 'project_resource'::text]))),
