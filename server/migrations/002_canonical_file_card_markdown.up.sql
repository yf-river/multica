-- Convert attachment-backed Markdown links to the one current file-card syntax
-- before the renderer drops hostname-based legacy detection.
DO $$
DECLARE
...
BEGIN
    ...
    LOOP
        ...
        SELECT string_agg(
            CASE
                WHEN btrim(line, E' \t\r\f\v') = old_marker THEN new_marker
                ELSE line
            END,
            E'\n' ORDER BY ordinal
        )
