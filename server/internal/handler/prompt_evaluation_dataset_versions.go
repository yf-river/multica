...
func (h *Handler) CreatePromptEvaluationDatasetFromTraces(w http.ResponseWriter, r *http.Request) {
	...
	tx, qtx, ok := h.beginResourceCreateTransaction(w, r.Context(), "failed to start trace dataset import transaction")
	if !ok {
		return
	}
...
func (h *Handler) CreatePromptEvaluationDatasetVersion(w http.ResponseWriter, r *http.Request) {
	...
	tx, qtx, ok := h.beginResourceCreateTransaction(w, r.Context(), "failed to start dataset version transaction")
	if !ok {
		return
	}
	defer func() { _ = tx.Rollback(r.Context()) }()
	...
	if err != nil {
		writeError(w, http.StatusInternalServerError, "failed to lock dataset asset")
		return
	}
	asset = lockedAsset
	version, err := h.createPromptEvaluationDatasetVersionFromCurrent(r.Context(), qtx, asset, actorID, req.VersionLabel, metadata)
	...
	response := promptEvaluationDatasetVersionToResponse(version)
	if err := completeResourceCreateRequest(
		r.Context(), qtx, asset.WorkspaceID, actorID, resourceTypePromptDatasetVersion,
		idempotencyKey, requestHash, version.ID, response,
	); err != nil {
		writeError(w, http.StatusInternalServerError, "failed to complete dataset version request")
		return
	}
	if err := tx.Commit(r.Context()); err != nil {
...
func (h *Handler) RestorePromptEvaluationDatasetVersion(w http.ResponseWriter, r *http.Request) {
	...
	tx, qtx, ok := h.beginResourceCreateTransaction(w, r.Context(), "failed to start dataset version restore transaction")
	if !ok {
		return
	}
	defer func() { _ = tx.Rollback(r.Context()) }()
