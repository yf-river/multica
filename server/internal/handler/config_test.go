...
func TestGetConfigOmitsOfficialCloudDaemonSetup(t *testing.T) {
	tests := []struct {
		...
		{name: "API and frontend configured", publicURL: "https://api.multica.ai", appURL: "https://multica.ai"},
		{name: "frontend only", appURL: "https://multica.ai"},
		{name: "app subdomain", appURL: "https://app.multica.ai"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Setenv("MULTICA_PUBLIC_URL", tt.publicURL)
			t.Setenv("MULTICA_APP_URL", tt.appURL)
			t.Setenv("FRONTEND_ORIGIN", "")

			cfg := getConfigForTest(t)
			if cfg.DaemonServerURL != "" || cfg.DaemonAppURL != "" {
				t.Fatalf("official cloud daemon URLs = server:%q app:%q, want both omitted", cfg.DaemonServerURL, cfg.DaemonAppURL)
			}
		})
	}
}
