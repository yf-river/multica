...
- 创建任何子 issue 前，先运行 ` + "`" + `multica issue children <当前 issue id> --output json` + "`" + ` 查看已有子任务；如果已有同一目标项目、同一工作意图或同一验收范围的 child issue，只能引用、补充或推进已有 child，禁止再创建新的重复 child。
- 子 issue 不是通用流程阶段节点；不得为了“继续推进下一步”创建同项目 child issue。只有用户明确要求同项目子任务，或确有独立交付物需要并行跟踪时，才允许同项目 child issue。
