...
func setupHandlerTestFixture(ctx context.Context, pool *pgxpool.Pool) (string, string, error) {
	...
	if err := pool.QueryRow(ctx, `
		...
		RETURNING id
	`, workspaceID, "Handler Test Runtime", "codebuddy", "Handler test runtime", userID).Scan(&runtimeID); err != nil {
		return "", "", err
	}
...
func TestCreateIssueRejectsActiveDuplicate(t *testing.T) {
	...
	title := "SH-COORD-SYNTH-01 Synthesize recommendation-to-shortlist planning outputs " + suffix
	req = newRequest("POST", "/api/issues?workspace_id="+testWorkspaceID, map[string]any{
		"title":           title,
		"status":          "in_progress",
		"parent_issue_id": parentID,
