...
func buildMetaSkillContent(provider string, ctx TaskContextForEnv) string {
	...
	if ctx.SourceSummaryPrompt != "" {
		...
		if ctx.ExecutionPolicy.IsNoRepoBoundedStage() && !ctx.IsSquadLeader {
			...
			fmt.Fprintf(&b, "2. Treat the triggering comment ID `%s` as the coordinator dispatch context. Do not query it with `multica issue comment list`; if a needed fact is absent from the prompt, record it as a handoff question or assumption.\n", ctx.TriggerCommentID)
			...
			if ctx.IsSquadLeader {
				...
				if isStageChainCoordinator(ctx) {
					b.WriteString("   - Stage-chain coordinator rule from your squad instructions: on comment-triggered runs, only review the latest stage output or human reply, then choose exactly one of wait, block, dispatch one next stage by `mention://agent/...`, or close after the configured stages. Do not implement, inspect repositories, create MRs, run tests, or simulate stages inside this coordinator task.\n")
					b.WriteString("   - Stage-chain transition rule: when the latest stage output is complete and unblocked, advance exactly one step in the configured chain. Do not repeat or paraphrase the completed stage handoff as your own final output; that leaves the next stage untriggered.\n")
...
func isStageChainCoordinator(ctx TaskContextForEnv) bool {
	if !ctx.IsSquadLeader || !ctx.ExecutionPolicy.IsCoordinator() {
		return false
	}
	instructions := ctx.AgentInstructions
	return strings.Contains(instructions, "stage_chain")
}
