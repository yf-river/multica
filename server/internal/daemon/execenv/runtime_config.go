...
func buildMetaSkillContent(provider string, ctx TaskContextForEnv) string {
	...
	b.WriteString("When this run produces a durable stage artifact (for example a 01-clarify requirement note, 02-design proposal, 03-task-split handoff, 04-implement change summary, or 05-verify report), save it as a UTF-8 `.md` file under `$MULTICA_ARTIFACT_DIR` when that environment variable is set; otherwise use `artifacts/multica/` in the current working directory. The daemon automatically uploads markdown artifacts from the managed artifact directory, `artifacts/`, and `.multica/artifacts/` to the issue before marking the task terminal, links them to an agent-authored issue comment, and the platform renders them for download or preview. Mention the artifact titles in your stage summary, but do not rely on local paths in prose. Temporary scratch files that are not useful to a future reader should not be saved there.\n\n")
