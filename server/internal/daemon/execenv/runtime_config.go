...
func buildMetaSkillContent(provider string, ctx TaskContextForEnv) string {
	...
	b.WriteString("- `multica issue pull-request link <issue-id> --provider gongfeng --html-url <mr-url> [--project-path <path>] [--iid <iid>] [--title <title>] [--source-branch <branch>] [--target-branch <branch>] --output json` — After opening or updating a Gongfeng MR, record it on the issue so the MR card and CodeReview handoff are visible in the platform.\n\n")
