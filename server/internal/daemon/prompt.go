...
func buildLifeJobPrompt(task Task) string {
	...
	b.WriteString("The context contains exact new materials plus compact long-term indexes, not the user's entire raw history. When an older material or chronicle may change the judgment, call `life_evidence_resolve` with its typed references before concluding. Do not guess unavailable evidence.\n")
