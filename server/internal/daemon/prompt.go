...
func buildLifeJobPrompt(task Task) string {
	...
	b.WriteString("The context contains exact new materials plus compact long-term indexes, not the user's entire raw history. Absence from an index never proves that something did not happen. Do not claim a first occurrence, ordinal count, or exact frequency unless the evidence explicitly enumerates a bounded complete scope; otherwise state the uncertainty. A user or companion message that says 'first', 'third', or another count is only that speaker's claim, not proof that the visible history is complete. When an indexed material, chronicle, memory, or observer knowledge item may change the judgment, call `life_evidence_resolve` before concluding. Resolver reference source_type is exactly material|chronicle|memory|observer_knowledge. Final output evidence source_type is exactly chat_message|task|comment|project|manual|external|memory|experiment_round|chronicle|observer_knowledge; after resolving a material, copy its actual source_type and ID instead of writing material. Do not guess unavailable evidence.\n")
	b.WriteString("For this background job, the only business operations available are `life_evidence_resolve` and `life_job_complete`. CodeBuddy may expose `ToolSearch` and `DeferExecuteTool` only to load and transport those MCP calls. Do not inspect the working directory, create files, call other life mutation commands, or inspect the product repository or source code to infer the contract. Internal thoughts and candidate memory judgements may be drafted and revised freely. Confirming, correcting, downgrading, or archiving a governed memory, and any change to tasks, experiments, modules, personality rules, or reality still requires the user's confirmation.\n")
	...
	b.WriteString("For every emitted item, object-valued fields are required JSON objects: internal_thoughts.metadata, proactive_decision.context_snapshot, experiment_review.module_proposal, and upgrade_evaluation.result. Use {} when the object is intentionally empty; never omit it or send null.\n")
	...
	case "understand_materials":
		b.WriteString("Review the new materials. Raw events already belong to materials and chronicles: never create a memory merely to restate what happened or what either person said. A temporary event, appointment, one-time constraint, or the mere existence of a conversation is not a memory even when factually true; leave it in materials or represent it through a topic, commitment, or chronicle only when that serves a real purpose. A memory is a durable, reusable understanding expected to matter in a different future situation. Producing no memory is the normal result when that standard is not met. When new evidence changes an existing candidate memory, return that supplied memory_id and revise it instead of creating another memory; never use the ID of a confirmed or archived memory. When evidence invalidates an active internal thought, return its supplied thought_id and revise it or set status=archived; never leave an active contradiction and merely add a correction beside it. Create only genuinely useful evidence-backed topics, commitments, relationship events, internal thoughts, or an event chronicle when their own durable purpose is met.\n")
	...
	case "observation_aggregate":
		b.WriteString("Act as the main companion, but do not filter or rewrite away an observer's disagreement. `published_observer_judgements` already contains the complete new judgements for this job: do not resolve those IDs as evidence. Put their IDs directly in observation_topics.judgement_ids. Group them into useful observation topics, preserve every linked judgement, and add no conclusion that lacks evidence.\n")
...
func lifeJobOutputContract(jobType string) string {
	...
	const memory = "memory_candidates:[{memory_id?:UUID,kind:string,content:string,confidence:number,urgency:number,uncertainty:string," + evidence + "}]"
	...
	const thought = "internal_thoughts:[{thought_id?:UUID,type:string,title:string,content:string,status:active|archived,metadata:object," + evidence + "}]"
...
func buildChatPrompt(task Task) string {
	...
	if strings.TrimSpace(task.LifeContext) != "" {
		...
		b.WriteString("\nUse this only as revisable background. It is a selected index, not a complete event list: absence never proves that something did not happen. Do not claim a first occurrence, ordinal count, or exact frequency unless the supplied evidence explicitly enumerates a bounded complete scope; otherwise state the uncertainty. A user or companion message that states a count is only that speaker's claim, not proof that the visible history is complete. The current message may update or contradict it. Do not turn a temporary feeling, impulse, or isolated expression into a fact, plan, commitment, or decision. Surface uncertainty and ask the user to confirm any durable new understanding before treating it as memory.\n\n")
	}
