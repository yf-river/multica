...
func buildLifeJobPrompt(task Task) string {
	...
	b.WriteString("The context contains exact new materials plus compact long-term indexes, not the user's entire raw history. When an indexed material, chronicle, memory, or observer knowledge item may change the judgment, call `life_evidence_resolve` before concluding. Resolver reference source_type is exactly material|chronicle|memory|observer_knowledge. Final output evidence source_type is exactly chat_message|task|comment|project|manual|external|memory|experiment_round|chronicle|observer_knowledge; after resolving a material, copy its actual source_type and ID instead of writing material. Do not guess unavailable evidence.\n")
