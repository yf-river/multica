...
func buildLifeJobPrompt(task Task) string {
	...
	b.WriteString("The context contains exact new materials plus compact long-term indexes, not the user's entire raw history. When an indexed material, chronicle, memory, or observer knowledge item may change the judgment, call `life_evidence_resolve` with its typed reference before concluding. Do not guess unavailable evidence.\n")
