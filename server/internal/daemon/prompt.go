...
func buildLifeJobPrompt(task Task) string {
	...
	b.WriteString("For this background job, the only business operations available are `life_evidence_resolve` and `life_job_complete`. CodeBuddy may expose `ToolSearch` and `DeferExecuteTool` only to load and transport those MCP calls. Do not inspect the working directory, create files, call other life mutation commands, or inspect the product repository or source code to infer the contract. Internal thoughts and drafts may be developed freely. Shared memories, tasks, experiments, modules, personality rules, and reality changes still require the user's confirmation.\n")
