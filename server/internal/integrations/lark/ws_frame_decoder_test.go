...
func TestDecodeLarkFrameTextMessageInP2P(t *testing.T) {
	t.Parallel()
	raw := []byte(`{
		"schema":"2.0",
		"header":{
	...
	msg, ok, err := DecodeLarkFrame(raw, db.LarkInstallation{BotOpenID: "ou_bot"})
	if err != nil || !ok {
...
func TestDecodeLarkFrameGroupMentionDiscrimination(t *testing.T) {
	t.Parallel()
	...
	t.Run("mentions bot", func(t *testing.T) {
		msg, ok, err := DecodeLarkFrame(mkRaw("ou_bot"), db.LarkInstallation{BotOpenID: "ou_bot"})
		if err != nil || !ok {
	...
	t.Run("mentions other user", func(t *testing.T) {
		msg, ok, err := DecodeLarkFrame(mkRaw("ou_other"), db.LarkInstallation{BotOpenID: "ou_bot"})
		if err != nil || !ok {
			t.Fatalf("ok=%v err=%v", ok, err)
		}
		if msg.AddressedToBot {
...
// TestDecodeLarkFrameGroupMentionUnionID exercises the MUL-2671
...
func TestDecodeLarkFrameGroupMentionUnionID(t *testing.T) {
	...
	t.Run("union_id match wins even when open_id mismatches", func(t *testing.T) {
		...
		msg, ok, err := DecodeLarkFrame(mkRaw("ou_bot_a_wire", "on_bot_a_union"), inst)
		if err != nil || !ok {
			t.Fatalf("ok=%v err=%v", ok, err)
		}
		if !msg.AddressedToBot {
	...
	t.Run("union_id mismatch wins even when open_id matches", func(t *testing.T) {
		...
		msg, ok, err := DecodeLarkFrame(mkRaw("ou_bot_a_canonical", "on_bot_b_union"), inst)
		if err != nil || !ok {
			t.Fatalf("ok=%v err=%v", ok, err)
		}
		if msg.AddressedToBot {
	...
	t.Run("falls back to open_id when union_id is unknown", func(t *testing.T) {
		...
		msg, ok, err := DecodeLarkFrame(mkRaw("ou_bot_a_canonical", "on_anything"), inst)
		if err != nil || !ok {
			t.Fatalf("ok=%v err=%v", ok, err)
		}
		if !msg.AddressedToBot {
...
// TestDecodeLarkFrameMentionPlaceholderRewrite covers the body
...
func TestDecodeLarkFrameMentionPlaceholderRewrite(t *testing.T) {
	t.Parallel()
	pgText := func(s string) pgtype.Text { return pgtype.Text{String: s, Valid: true} }
	...
	t.Run("strips bot self-mention via union_id", func(t *testing.T) {
		...
		msg, ok, err := DecodeLarkFrame(mkRaw("@_user_1 ping test", mentions), inst)
		if err != nil || !ok {
			t.Fatalf("ok=%v err=%v", ok, err)
		}
	...
	t.Run("substitutes other-user mention with display name", func(t *testing.T) {
		...
		msg, ok, err := DecodeLarkFrame(mkRaw("@_user_1 hey @_user_2 take a look", mentions), inst)
		if err != nil || !ok {
			t.Fatalf("ok=%v err=%v", ok, err)
		}
	...
	t.Run("preserves newlines after stripped mention", func(t *testing.T) {
		...
		mentions := `[{"key":"@_user_1","name":"My Bot","id":{"open_id":"ou_bot_wire","union_id":"on_bot"}}]`
		msg, ok, err := DecodeLarkFrame(mkRaw("@_user_1  first line\nsecond line", mentions), inst)
		if err != nil || !ok {
			t.Fatalf("ok=%v err=%v", ok, err)
		}
	...
	t.Run("no mentions leaves body unchanged", func(t *testing.T) {
		...
		msg, ok, err := DecodeLarkFrame(mkRaw("just a normal message", `[]`), inst)
		if err != nil || !ok {
			t.Fatalf("ok=%v err=%v", ok, err)
		}
	...
	t.Run("preserves indentation and tabs around stripped mention", func(t *testing.T) {
		...
		msg, ok, err := DecodeLarkFrame(mkRaw(raw, mentions), inst)
		if err != nil || !ok {
			t.Fatalf("ok=%v err=%v", ok, err)
		}
		if msg.Body != want {
...
func TestDecodeLarkFrameDropsHeartbeat(t *testing.T) {
	t.Parallel()
	cases := [][]byte{
		[]byte(`{"type":"heartbeat"}`),
	...
	for _, raw := range cases {
		msg, ok, err := DecodeLarkFrame(raw, db.LarkInstallation{})
		if err != nil || ok {
...
func TestDecodeLarkFrameEmptyRaw(t *testing.T) {
	t.Parallel()
	msg, ok, err := DecodeLarkFrame(nil, db.LarkInstallation{})
	if ok || err != nil {
		t.Fatalf("expected (zero, false, nil) for empty raw; got ok=%v err=%v msg=%+v", ok, err, msg)
	}
}

func TestDecodeLarkFrameMalformedReturnsError(t *testing.T) {
	t.Parallel()
	_, ok, err := DecodeLarkFrame([]byte("not-json"), db.LarkInstallation{})
	if err == nil {
...
func TestDecodeLarkFrameMessageContentEmptyOnInvalidContentJSON(t *testing.T) {
	t.Parallel()
	raw := []byte(`{
		"schema":"2.0",
		"header":{"event_id":"e","event_type":"im.message.receive_v1","app_id":"a"},
...
func TestDecodeLarkFrameNonTextMessageHasEmptyBody(t *testing.T) {
	t.Parallel()
	raw := []byte(`{
		"schema":"2.0",
		"header":{"event_id":"e","event_type":"im.message.receive_v1","app_id":"a"},
...
// TestDecodeLarkFramePostMessageFlattened verifies that a rich-text
...
func TestDecodeLarkFramePostMessageFlattened(t *testing.T) {
	t.Parallel()
...
// TestDecodeLarkFramePostResolvesMentions checks that @-mentions in
...
// bot's own mention.
func TestDecodeLarkFramePostResolvesMentions(t *testing.T) {
	t.Parallel()
...
// TestDecodeLarkFrameCapturesReplyLinkage verifies parent_id /
// root_id from a quote-reply event land on the InboundMessage so the
// enricher can expand them.
func TestDecodeLarkFrameCapturesReplyLinkage(t *testing.T) {
	t.Parallel()
	raw := []byte(`{
		"schema":"2.0",
		"header":{"event_id":"e","event_type":"im.message.receive_v1","app_id":"a"},
