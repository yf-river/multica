...
// Task retry / fail / rerun flows.
// Split out of task.go — pure code move, no logic change.
//
// FailTask is the single write-path that lands a task in the failed state;
// MaybeRetryFailedTask decides whether to enqueue a retry; RerunIssue and
// enqueueRerunTask power the "rerun this issue" UX; HandleFailedTasks is the
// reconciler entry-point invoked when the daemon notices stranded failures.
//
// retryableReasons / taskRetryBudget / resumeUnsafeFailureReason are the
// policy tables for that decision.
...
//
// failureReason is the classifier consumed by the auto-retry path. Pass ""
// when unknown; the server classifies the raw error text before persisting.
