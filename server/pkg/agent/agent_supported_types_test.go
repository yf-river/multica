...
// protocol_family CHECK are aligned to. If a backend is added to New, it must
// be added here too — and to the current schema CHECK.
...
// TestSupportedTypesMatchesSchemaWhitelist pins the exact set so a drift from
// the runtime_profile.protocol_family CHECK fails loudly.
func TestSupportedTypesMatchesSchemaWhitelist(t *testing.T) {
	...
	if len(SupportedTypes) != len(want) {
		t.Fatalf("SupportedTypes has %d entries, schema whitelist has %d; keep them in lockstep", len(SupportedTypes), len(want))
	}
	for _, typ := range SupportedTypes {
		if !want[typ] {
			t.Errorf("SupportedTypes contains %q which is not in the current protocol_family CHECK", typ)
		}
	}
}
