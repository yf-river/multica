...
VALUES (sqlc.arg(event_type), sqlc.narg(stream_key), sqlc.narg(workspace_id), sqlc.narg(actor_type), sqlc.narg(actor_id), sqlc.narg(task_id), sqlc.narg(chat_session_id), sqlc.arg(payload))
RETURNING *;
...
SET lease_owner = sqlc.arg(lease_owner), lease_until = now() + sqlc.arg(lease_duration)::interval
FROM picked WHERE event.id = picked.id RETURNING event.*;

-- name: HasDomainEventDelivery :one
SELECT EXISTS (SELECT 1 FROM domain_event_delivery WHERE event_id = sqlc.arg(event_id) AND consumer = sqlc.arg(consumer));

-- name: RecordDomainEventDelivery :exec
INSERT INTO domain_event_delivery (event_id, consumer) VALUES (sqlc.arg(event_id), sqlc.arg(consumer)) ON CONFLICT DO NOTHING;
...
WHERE id = sqlc.arg(id) AND lease_owner = sqlc.arg(lease_owner) AND dead_lettered_at IS NULL;

-- name: RetryDomainEvent :execrows
UPDATE domain_event_outbox SET attempts = attempts + 1, available_at = now() + sqlc.arg(retry_delay)::interval,
  lease_owner = NULL, lease_until = NULL, last_error = sqlc.arg(last_error)
WHERE id = sqlc.arg(id) AND lease_owner = sqlc.arg(lease_owner) AND dead_lettered_at IS NULL;

-- name: DeadLetterDomainEvent :execrows
UPDATE domain_event_outbox SET attempts = attempts + 1, dead_lettered_at = now(), dead_letter_reason = sqlc.arg(dead_letter_reason),
  lease_owner = NULL, lease_until = NULL, last_error = sqlc.arg(dead_letter_reason)
WHERE id = sqlc.arg(id) AND lease_owner = sqlc.arg(lease_owner) AND processed_at IS NULL AND dead_lettered_at IS NULL;
...
DELETE FROM domain_event_outbox event WHERE event.id IN (
  ...
  ORDER BY COALESCE(candidate.processed_at, candidate.dead_lettered_at), candidate.sequence_no LIMIT sqlc.arg(batch_size)
);
