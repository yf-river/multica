...
const issue = post("/api/issues", {
  workspace_id: workspace.id,
  title: `curl Codex 小队端到端验收 ${suffix}`,
  description: "请用中文完成一次最小验收：说明你已收到任务，输出 trace/任务标识占位、验收证据和下一步。不要修改代码。",
  status: "todo",
  priority: "medium",
  assignee_type: "squad",
...
const terminalTask = await poll(async () => {
  const tasks = get(`/api/issues/${issue.id}/task-runs`, token);
  const items = Array.isArray(tasks) ? tasks : tasks.items ?? [];
  const task = items.find((item) => item.agent_id === agent.id || item.assignee_id === agent.id) || items[0];
