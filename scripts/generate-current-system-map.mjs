...
const OUTPUT_JSON = "artifacts/code-health/current-system-map.json";
const OUTPUT_MARKDOWN = "artifacts/code-health/current-system-map.md";
const OUTPUT_SUMMARY = "docs/architecture/current-system-summary.md";
  ...
  for (const match of content.matchAll(pattern)) {
    ...
    rewrites.push({
      ...
      environment: usesDocs
        ? ["DOCS_URL"]
        : usesRemoteAPI
          ? ["REMOTE_API_URL", "NEXT_PUBLIC_API_URL", "BACKEND_PORT", "PORT"]
          : [],
...
function renderMarkdown(systemMap) {
  ...
  return `# Expanded current-system inventory

> Generated by \`pnpm generate:current-system-map\` under the ignored
> \`artifacts/code-health/\` directory. Do not edit or commit this expanded
> evidence. Edit source code or \`current-system-map-overrides.json\`, then
> regenerate it when detailed inspection is needed.

This document is the deterministic static inventory that feeds the maintained
architecture summary; it is not, by itself, a claim that domain calls and
recovery semantics are fully documented. CI rebuilds the inventory, checks the
compact tracked summary and runs semantic inventory tests. The JSON companion
contains every discovered record for local or CI artifact inspection.
...
function renderSummary(systemMap) {
  const s = systemMap.summary;
  return `# Current system summary
...
| Database triggers | ${s.databaseTriggers} |
| Database indexes | ${s.databaseIndexes} |
| Migration files | ${s.migrations} |
| sqlc modules | ${s.sqlcModules} |
| sqlc queries | ${s.sqlcQueries} |
| Go WebSocket events | ${s.goWebsocketEvents} |
| TypeScript WebSocket events | ${s.typescriptWebsocketEvents} |
| Zustand stores | ${s.zustandStores} |
| React Query consumer files | ${s.reactQueryConsumerFiles} |
...
const summaryOutput = renderSummary(systemMap);
const results = checkOnly
  ? [writeOrCheck(OUTPUT_SUMMARY, summaryOutput)]
  : [
...
if (checkOnly) {
  ...
  console.log(`generated ${OUTPUT_JSON}, ${OUTPUT_MARKDOWN}, and ${OUTPUT_SUMMARY}`);
}
