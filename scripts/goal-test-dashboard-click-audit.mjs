...
} = process.env.MULTICA_NAV_AUDIT_LOCAL === "1"
  ? loadLocalBrowserAudit()
  : loadGoalTestBrowserAudit();
...
const dashboardClicks = [
  ...
  { id: "usage", label: "统计", link: "统计", path: `/${workspaceSlug}/usage`, ready: { heading: "统计" } },
...
try {
  ...
  setup = await openStartPage(page);
  clicks = setup.ok ? await auditClicks(page) : [];
} finally {
  await browser.close();
}

const deploymentLogs = process.env.MULTICA_NAV_AUDIT_LOCAL === "1"
  ? { ok: true, target: "local", skipped: true }
  : verifyGoalTestDeploymentLogs(env);
...
const payload = {
  ...
  thresholds: {
    ...
    max_repeat_click_ms: maxRepeatClickMs,
...
async function auditClicks(page) {
  ...
  for (const item of dashboardClicks) {
    ...
    results.push(await measureClick(page, item, "first", maxClickMs));
    await resetNavigationBaseline(page, item);
    results.push(await measureClick(page, item, "repeat", maxRepeatClickMs));
  }
  return results;
}
...
async function resetNavigationBaseline(page, item) {
  ...
  if (new URL(page.url(), browserURL).pathname !== baselinePath) {
    await clickNavigationLink(
      ...
      { nativeTimeoutMs: 10_000, retryTimeoutMs: 15_000 },
    );
  }
...
async function measureClick(page, item, phase, maxReadyMs) {
  const initialLink = await locateNavigationLink(page, item);
  const initialDescriptor = await describeNavigationLink(initialLink);
  ...
  try {
    navigationClick = await clickNavigationLink(page, item, {
      nativeTimeoutMs: maxReadyMs,
      retryTimeoutMs: maxReadyMs,
      initialLink,
      initialDescriptor,
    });
  ...
  const failures = [
    ...
    ...(readyMs > maxReadyMs ? [`点击可用耗时 ${readyMs}ms 超过 ${maxReadyMs}ms`] : []),
...
async function clickNavigationLink(
  ...
  try {
    const link = initialLink ?? await locateNavigationLink(page, item);
    const descriptor = initialDescriptor ?? await describeNavigationLink(link);
...
function renderMarkdown(payload) {
  const lines = [
    ...
    "## 最慢点击",
    "",
    ...payload.summary.slowest_clicks.map((click) => `- ${click.label}（${click.phase === "repeat" ? "再次点击" : "首次点击"}）：可用 ${click.ready_ms}ms，总等待 ${click.total_ms}ms，API ${click.api_request_count}`),
    "",
  ];
  ...
  for (const click of payload.clicks) {
    lines.push(`### ${click.ok ? "通过" : "失败"}：${click.label}（${click.phase === "repeat" ? "再次点击" : "首次点击"}）`);
