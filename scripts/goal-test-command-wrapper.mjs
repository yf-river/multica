...
function isSafeRTKCommand(command) {
  ...
  return /\b(go test|pnpm (?:--filter [^ ]+ )?(?:exec playwright test|test|typecheck)|make goal-test-(?:smoke|e2e|e2e-all|real-agent-e2e|ui-audit|training-performance-audit)|node scripts\/goal-test-environments\.mjs verify(?:-logs)? )/.test(normalized);
}
