...
function isSafeRTKCommand(command) {
  ...
  return /\b(go test|pnpm (?:--filter [^ ]+ )?(?:exec playwright test|test|typecheck)|make goal-test-(?:smoke|ui-audit|training-performance-audit)|node scripts\/goal-test-environments\.mjs verify(?:-logs)? )/.test(normalized);
}
