...
function isSafeRTKCommand(command) {
  ...
  return /\b(go test|pnpm (?:--filter [^ ]+ )?(?:exec playwright test|test|typecheck)|make goal-test-(?:smoke|e2e|e2e-all|real-agent-e2e|ui-audit|sop-browser-audit|training-performance-audit|public-training-performance-audit|playground-difference-audit)|node scripts\/goal-test-environments\.mjs verify(?:-logs)? )/.test(normalized);
}
