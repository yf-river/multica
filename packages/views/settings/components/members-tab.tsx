  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="text-xs text-muted-foreground truncate">{member.account}</div>
      </div>
      {showMenu && (
        <DropdownMenu>
          <DropdownMenuTrigger
...
export function MembersTab() {
  ...
  const handleCreateMember = async () => {
    ...
    try {
      ...
      toast.success(t(($) => $.members.toast_user_added));
    } catch (e) {
      toast.error(e instanceof Error ? e.message : t(($) => $.members.toast_user_add_failed));
    } finally {
          ...
          <Card>
            <CardContent className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-medium">{t(($) => $.members.add_user_title)}</h3>
              </div>
              <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_120px_auto]">
                <Input
                  value={newAccount}
                  onChange={(e) => setNewAccount(e.target.value)}
                  placeholder={t(($) => $.members.account_placeholder)}
                  onKeyDown={(e) => {
                    if (e.key === "Enter" && newAccount.trim()) handleCreateMember();
                  }}
                />
                <Input
                  value={newName}
                ...
                <Input
                  ...
                  placeholder={t(($) => $.members.password_placeholder)}
                  onKeyDown={(e) => {
                    if (e.key === "Enter" && newAccount.trim()) handleCreateMember();
                  }}
                />
                <Select value={newRole} onValueChange={(value) => setNewRole(value as MemberRole)}>
                  <SelectTrigger size="sm">
                    <SelectValue>{() => roleConfig[newRole].label}</SelectValue>
                  </SelectTrigger>
                  <SelectContent>
                ...
                <Button
                  ...
                  disabled={createLoading || !newAccount.trim()}
                >
