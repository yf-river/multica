  ...
  return (
    <dl className="grid grid-cols-2 gap-x-4 gap-y-3 border-t pt-3 sm:grid-cols-4">
      <Insight
        ...
        sub={busiestDay ? formatUsageCost(busiestDay.cost) : null}
      />
      <Insight
        label="Most active weekday"
        value={busyDayName ?? "—"}
        sub={busyDayName ? `avg ${formatUsageCost(busyDayAvg)}` : null}
      />
      <Insight
        label="Quietest weekday"
        value={quietDayName ?? "—"}
        sub={quietDayName ? `avg ${formatUsageCost(quietDayAvg)}` : null}
      />
      <Insight label={`${windowDays}-day total`} value={formatUsageCost(totalCost)} />
    </dl>
  );
}
