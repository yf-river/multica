      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 overflow-hidden lg:grid-cols-[360px_minmax(0,1fr)]">
        <aside className="min-h-0 border-b lg:border-r lg:border-b-0">
          <div className="flex h-10 items-center justify-between border-b px-3">
            <div className="text-caption font-medium text-muted-foreground">{t(($) => $.issue_runs)}</div>
          ...
          <div className="max-h-[38vh] min-h-0 overflow-y-auto lg:max-h-none lg:h-[calc(100vh-7.5rem)]">
            {issuesQuery.isLoading ? (
              ...
              <div className="px-3 py-6 text-body text-muted-foreground">
                {t(($) => $.empty_issues)}
              </div>
            ) : (
        ...
        <main className="min-h-0 overflow-y-auto">
          {selectedIssue ? (
            ...
            <div className="px-6 py-10 text-body text-muted-foreground">
              {t(($) => $.select_issue)}
            </div>
          )}
        </main>
...
function IssueRunRow({ issue, active, href }: { issue: Issue; active: boolean; href: string }) {
  ...
  return (
    <AppLink
      href={href}
      className={cn(
        "block px-3 py-2.5 text-left text-body hover:bg-accent/60",
    ...
    >
      <div className="flex items-start justify-between gap-3">
        ...
        {activity && activityLabel && (
          <span className={cn("shrink-0 rounded border px-1.5 py-0.5 text-caption", activity.tone === "running" ? "border-info/40 text-info" : "text-muted-foreground")}>
            {activityLabel}
          </span>
        )}
      </div>
    ...
    <div className="space-y-4 px-4 py-4">
      <section className="rounded-md border bg-card">
        <div className="flex flex-col gap-3 border-b px-4 py-3 lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0">
            <div className="text-caption text-muted-foreground">{issue.identifier}</div>
            <h2 className="mt-0.5 truncate text-title-sm font-semibold">{issue.title}</h2>
          ...
          <div className="flex flex-wrap gap-2">
            <AppLink className="rounded border px-2.5 py-1.5 text-caption hover:bg-accent" href={issueHref}>
        ...
        <div className="grid gap-3 border-t p-4 text-body md:grid-cols-3">
          <Metric
      ...
      <section className="rounded-md border bg-card">
          ...
          <table className="w-full table-fixed text-body">
            ...
            <tbody className="divide-y">
              {agentNodeRows.length > 0 ? agentNodeRows.map((row) => (
                ...
                <tr>
                  <td className="px-3 py-5 text-body text-muted-foreground" colSpan={6}>{t(($) => $.nodes.empty)}</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <div className="divide-y md:hidden">
          {agentNodeRows.length > 0 ? agentNodeRows.map((row) => (
            <div key={row.key} className="px-4 py-3 text-body">
              <div className="min-w-0 truncate font-medium">{agentNodeDisplayLabel(row)}</div>
              <div className="mt-2 grid grid-cols-2 gap-x-3 gap-y-1 text-caption text-muted-foreground">
            ...
            <div className="px-4 py-5 text-body text-muted-foreground">{t(($) => $.nodes.empty)}</div>
          )}
        </div>
      </section>
      ...
      <section className="rounded-md border bg-card">
        ...
        <div className="border-y p-3">
          <input
            className="w-full rounded-md border bg-background px-3 py-2 text-body outline-none focus:border-ring"
            value={eventQuery}
        ...
        <div className="min-h-[24rem] space-y-3 p-3">
          {visibleEventGroups.length > 0 ? visibleEventGroups.map((group, index) => (
            ...
            <div className="flex gap-2 rounded-md border border-dashed bg-muted/20 px-3 py-6 text-body text-muted-foreground">
      ...
      <div className="flex items-start gap-3 border-b bg-muted/45 px-3 py-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex min-w-0 flex-wrap items-center gap-2">
            ...
            <span className={cn("shrink-0 rounded border px-1.5 py-0.5 text-caption", eventGroupOutcomeClass(group))}>{group.outcome}</span>
          </div>
          <div className="mt-2 flex flex-wrap gap-1.5 text-caption text-muted-foreground">
...
function RunReviewEventRow({
  ...
  return (
    <article
      className="cursor-pointer rounded-md border bg-background p-3 text-body shadow-sm transition-colors hover:bg-accent/25"
    ...
    >
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex min-w-0 flex-wrap items-center gap-2">
            <span className={cn("shrink-0 rounded border px-1.5 py-0.5 text-caption", tone.chip)}>{event.category}</span>
            <span className={cn("shrink-0 rounded border px-1.5 py-0.5 text-caption", tone.outcome)}>{event.outcome}</span>
            <span className="min-w-[12rem] flex-1 truncate font-medium">{event.title}</span>
            {event.timeLabel && <span className="shrink-0 text-caption text-muted-foreground">{event.timeLabel}</span>}
          </div>
          {event.summary && (
            <div className={cn("mt-2 rounded border px-2 py-1.5 text-caption leading-5", tone.summary)}>
              {event.summary}
            </div>
          )}
          <div className="mt-2 flex flex-wrap gap-1.5 text-caption text-muted-foreground">
  ...
  return (
    <Dialog open={Boolean(event)} onOpenChange={onOpenChange}>
      <DialogContent className="flex max-h-[88vh] w-[calc(100vw-2rem)] max-w-6xl flex-col gap-0 p-0 sm:!max-w-6xl lg:w-[calc(100vw-4rem)]">
        ...
        <div className="min-h-0 flex-1 space-y-3 overflow-auto bg-muted/10 p-5 text-body">
          {event ? (
            <>
...
function EventRawBlock({ title, value }: { title: string; value: string }) {
  return (
    <section className="rounded-md border bg-background shadow-sm">
      <div className="border-b px-3 py-2 text-caption font-medium text-muted-foreground">{title}</div>
      <pre className="max-h-[60vh] overflow-auto whitespace-pre px-3 py-2 text-caption leading-5">{value}</pre>
    </section>
  );
}
...
function RunFailureBanner({
  ...
  return (
    <div className="flex flex-col gap-3 border-b bg-destructive/5 px-4 py-3 text-body md:flex-row md:items-center md:justify-between">
      ...
      <button
        type="button"
        className="inline-flex shrink-0 items-center justify-center gap-1.5 rounded border border-destructive/30 bg-background px-2.5 py-1.5 text-caption text-destructive hover:bg-destructive/10 disabled:cursor-not-allowed disabled:opacity-60"
        onClick={onRetry}
        disabled={retrying}
...
function Metric({ label, value, detail }: { label: string; value: string; detail: string }) {
  return (
    <div className="min-w-0 rounded-md border bg-background px-4 py-3 shadow-sm">
      <div className="truncate text-body font-medium text-muted-foreground">{label}</div>
      <div className="mt-2 truncate text-display-sm font-semibold leading-none tracking-normal text-foreground">{value}</div>
      <div className="mt-3 truncate text-body text-muted-foreground" title={detail}>{detail}</div>
    </div>
  );
}
...
function ExportButton({ label, onClick }: { label: string; onClick: () => void }) {
  return (
    <button
      type="button"
      className="inline-flex items-center gap-1.5 rounded border bg-background px-2.5 py-1.5 text-caption text-muted-foreground hover:bg-accent hover:text-foreground"
      onClick={onClick}
    >
    ...
    <div className="flex min-w-0 flex-wrap gap-1.5">
      {uniqueArtifacts.slice(0, 3).map((artifact) => (
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="inline-flex max-w-full items-center rounded border bg-background px-2 py-0.5 text-caption text-muted-foreground hover:bg-accent hover:text-foreground"
      ...
      {uniqueArtifacts.length > 3 ? (
        <span className="inline-flex items-center rounded border px-2 py-0.5 text-caption text-muted-foreground">
          +{uniqueArtifacts.length - 3}
        </span>
      ) : null}
    </div>
...
function TimelineLaneChart({
  ...
  return (
    <div className="px-4 pb-4" data-testid="run-review-horizontal-timeline">
      <div className="grid grid-cols-[6.5rem_minmax(0,1fr)] gap-x-3 text-caption text-muted-foreground">
        <div />
        ...
        <div className="mt-2 space-y-1.5">
          {rows.length > 0 ? rows.map((row) => (
            <div key={row.key} className="grid grid-cols-[6.5rem_minmax(0,1fr)] items-center gap-x-3">
              <div className="min-w-0">
                <div className="truncate text-caption font-medium">{row.label}</div>
                <div className="truncate text-caption text-muted-foreground">{row.subtitle}</div>
              </div>
              <div className="relative h-9 overflow-hidden rounded-md border bg-muted/20">
                ...
                {row.missing || row.segments.length === 0 || row.segments.every((segment) => segment.startMs === null || segment.endMs === null) ? (
                  <div className="flex h-full items-center px-2 text-caption text-muted-foreground">
                      ...
                      <Tooltip key={segment.key}>
                        <TooltipTrigger
                          render={
                            <div
                              className={cn(
                                "absolute top-1 bottom-1 overflow-hidden rounded px-2 text-caption leading-7 shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-1",
            ...
            <div className="rounded-md border border-dashed bg-muted/20 px-3 py-4 text-body text-muted-foreground">
              {t(($) => $.timeline.empty)}
            </div>
          )}
        </div>
