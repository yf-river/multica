    ...
    <div data-testid="life-shell" className="flex h-full min-h-0 flex-col bg-background">
      <PageHeader className="h-16 shrink-0 justify-between border-b border-border/60 bg-background px-4 md:px-8">
        <div className="flex min-w-0 items-center gap-2 text-sm">
          <span className="text-muted-foreground">AI Studio 工作区</span><ChevronRight className="size-3 text-muted-foreground/50" aria-hidden="true" /><span className="shrink-0 font-medium">{t(($) => $.page.title)}</span>
        </div>
      ...
      <main className="min-h-0 flex-1 overflow-y-auto bg-background"><h1 className="sr-only">{t(($) => $.page.title)}</h1>
        <div className="mx-auto w-full max-w-[1150px] px-4 py-8 md:px-8 md:py-10">
          <header className="mb-8 flex items-start justify-between gap-6"><div className="max-w-2xl space-y-2"><p className="text-xs font-medium tracking-wide text-brand">人生 / 长期关系</p><h2 className="text-base font-medium tracking-tight">{tabCopy[activeTab].label}</h2><p className="text-sm leading-6 text-muted-foreground">{tabCopy[activeTab].description}</p></div><Button className="hidden shrink-0 md:inline-flex" size="sm" variant="outline" onClick={() => (document.getElementById(`life-${activeTab}-details`) ?? document.querySelector(`[data-testid=life-${activeTab}-panel]`))?.scrollIntoView({ behavior: "smooth" })}>+ {actionLabel}</Button></header>
...
function QuickMaterialRecorder() {
  ...
  return <section data-testid="memory-quick-record" className="mb-4 rounded-lg border border-dashed border-border/70 bg-muted/20 p-3"><div className="flex flex-wrap items-center gap-2"><span className="shrink-0 text-xs font-medium text-muted-foreground">{t(($) => $.complete.material_title)}</span><Input className="h-8 min-w-0 flex-1 bg-background" value={material} onChange={(event) => setMaterial(event.target.value)} placeholder={t(($) => $.complete.material_placeholder)} /><Button size="sm" disabled={!material.trim() || add.isPending} onClick={() => add.mutate()}><Plus />{t(($) => $.complete.record)}</Button></div></section>;
}
...
function MemoryPanel() {
  ...
  return <div id="life-memory-details" data-testid="life-memory-panel" className="space-y-8">
    <div className="grid items-start gap-5 lg:grid-cols-[1.58fr_.82fr]">
      <section data-testid="memory-focus-card" className="rounded-xl border border-brand/20 bg-brand/[0.04] p-5 md:p-7"><div className="flex items-center justify-between"><div className="flex items-center gap-2 text-xs font-medium text-brand"><Sparkles className="size-4" />长期理解</div>{focus && <span className="rounded-full bg-background/80 px-2 py-1 text-xs text-muted-foreground">{focus.status === "confirmed" ? t(($) => $.memory.confirmed) : t(($) => $.memory.candidate)}</span>}</div>{focus ? <><h2 className="mt-5 line-clamp-4 text-base font-medium leading-7">{focus.content}</h2><div className="mt-5 flex flex-wrap gap-2">{focus.evidence.slice(0, 4).map((evidence) => <span key={`${evidence.source_type}:${evidence.source_id}`} className="rounded-full border border-border/60 bg-background px-2.5 py-1 text-xs text-muted-foreground">{lifeEvidenceLabel(evidence.source_type)}</span>)}<span className="rounded-full border border-border/60 bg-background px-2.5 py-1 text-xs text-muted-foreground">可信度 {Math.round(focus.confidence * 100)}%</span></div>{focus.uncertainty && <p className="mt-4 line-clamp-3 text-xs leading-5 text-muted-foreground">不确定：{focus.uncertainty}</p>}<div className="mt-6 flex flex-wrap gap-2"><MemoryRow memory={focus} compact showContent={false} /></div></> : <div className="mt-5 space-y-2"><p className="text-sm text-muted-foreground">{t(($) => $.memory.empty)}</p><p className="text-xs text-muted-foreground">先在下方记录一条材料，搭子会和你一起形成理解。</p></div>}</section>
    ...
    <section className="space-y-3"><div className="flex items-center justify-between"><h2 className="text-sm font-medium">其他正在使用的理解</h2><span className="text-xs text-muted-foreground">{others.length} 条</span></div>{others.length === 0 ? <p className="rounded-xl border border-dashed border-border p-6 text-sm text-muted-foreground">暂无其他理解。</p> : <div className="grid gap-3 md:grid-cols-2">{others.slice(0, 8).map((memory) => <MemoryRow key={memory.id} memory={memory} compact />)}{others.length > 8 && <details className="rounded-xl border border-dashed border-border p-4 md:col-span-2"><summary className="cursor-pointer text-sm text-muted-foreground">查看其余 {others.length - 8} 条理解</summary><div className="mt-3 grid gap-3 md:grid-cols-2">{others.slice(8).map((memory) => <MemoryRow key={memory.id} memory={memory} compact />)}</div></details>}</div>}</section>
...
function ObserversPanel() {
  ...
  return <div id="life-observers-details" data-testid="life-observers-panel" className="space-y-8">
    ...
    <section className="rounded-xl border border-brand/20 bg-brand/[0.04] p-5 md:p-6"><p className="text-sm font-medium">第三方视角 · 保留分歧</p><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">他们可以看见人生系统中的全部信息，但各自独立判断。你不需要接受每一个结论，分歧本身也是值得保留的线索。</p></section>
