...
function ObserversPanel() {
  ...
  return <div className="mx-auto max-w-3xl space-y-8">
    ...
    <section className="space-y-3"><h2 className="text-sm font-medium">{t(($) => $.complete.background_title)}</h2><p className="text-xs text-muted-foreground">{t(($) => $.complete.recent_jobs, { value: (jobsQuery.data?.jobs ?? []).slice(0, 8).map((job) => `${job.job_type}:${job.status}`).join(" · ") || t(($) => $.complete.none) })}</p>{(upgradesQuery.data?.evaluations ?? []).map((item) => <article key={item.id} className="rounded-lg border p-4 text-sm"><div>{t(($) => $.complete.comparison, { candidate: item.candidate_label, baseline: item.baseline_label })}</div><div className="mt-1 text-xs text-muted-foreground">{item.status}{item.rollback_recommended ? ` · ${t(($) => $.complete.rollback)}` : ""}</div></article>)}<details className="rounded-lg border p-4"><summary className="cursor-pointer text-sm">{t(($) => $.complete.upgrade_title)}</summary><div className="mt-3 grid gap-2"><Input placeholder={t(($) => $.complete.candidate_label)} value={upgradeDraft.candidate_label} onChange={(event) => setUpgradeDraft({ ...upgradeDraft, candidate_label: event.target.value })}/><Input placeholder={t(($) => $.complete.baseline_label)} value={upgradeDraft.baseline_label} onChange={(event) => setUpgradeDraft({ ...upgradeDraft, baseline_label: event.target.value })}/><Textarea placeholder={t(($) => $.complete.scenarios_json)} value={upgradeDraft.scenarios} onChange={(event) => setUpgradeDraft({ ...upgradeDraft, scenarios: event.target.value })}/><Button size="sm" disabled={!upgradeDraft.candidate_label.trim() || createUpgrade.isPending} onClick={() => createUpgrade.mutate()}>{t(($) => $.complete.start_upgrade)}</Button></div></details></section>
  </div>;
}
