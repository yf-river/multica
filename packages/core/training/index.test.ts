...
describe("training workbench navigation", () => {
  it("uses the demo dashboard as the production default entry", () => {
    expect(DEFAULT_TRAINING_WORKBENCH_TAB).toBe("生产看板");
    expect(DEFAULT_TRAINING_WORKBENCH_VIEW).toBe("demo-dashboard");
    expect(trainingWorkbenchTabFromView(null)).toBe("生产看板");
    expect(trainingWorkbenchTabFromView("missing-view")).toBe("生产看板");
  });
