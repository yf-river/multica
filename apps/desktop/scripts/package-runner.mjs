 ...
 */
function stripLeadingSeparator(argv) {
...
function envWithLocalBins(env, root = desktopRoot) {
  const pathKey =
...
function resolveBuildMatrix(parsed, platform, arch) {
...
export function main({ argv, platform, arch, env }) {
  const passthrough = stripLeadingSeparator(argv);
  ...
  const disableMacNotarize = !env.APPLE_TEAM_ID;
