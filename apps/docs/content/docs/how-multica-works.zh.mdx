...
- **AI 编程工具**——[Antigravity](/providers#antigravity)、[Claude Code](/providers#claude-code)、[Codex](/providers#codex)、[Cursor](/providers#cursor)、[Copilot](/providers#copilot)、[Gemini](/providers#gemini)、[Hermes](/providers#hermes)、[Kimi](/providers#kimi)、[Kiro CLI](/providers#kiro-cli)、[OpenCode](/providers#opencode)、[Pi](/providers#pi) 11 款之一（或多款并存）。守护进程领到任务后，用这些工具真正去写代码。
...
默认使用**本地守护进程**执行任务。云端运行时由团队管理员统一配置；工作区启用后，成员可以直接选择云端环境，不需要留下联系方式。

## 下一步

- [团队快速上手](/cloud-quickstart) —— 5 分钟接入团队内部 Multica
- [自部署快速上手](/self-host-quickstart) —— 在自己的服务器上跑一套
