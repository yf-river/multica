...
## 3. Point the CLI at your own server
...
### 3b. Cross-machine: front with a reverse proxy
...
The chart **does not template secret values**. It references a Secret named `multica-secrets` by name, so real JWT, DB, and CloudFront keys never need to live in git or in `values.yaml`. Create the namespace + Secret once with kubectl:
...
Defaults assume the hostnames `multica.dev.lan` (web) and `api.multica.dev.lan` (backend). Add them to `/etc/hosts` (or local DNS) pointing at any node IP where your Ingress is reachable. To use different hostnames, copy `deploy/helm/multica/values.yaml`, edit `ingress.frontend.host` / `ingress.backend.host` and the matching `backend.config.appUrl` / `frontendOrigin` / `localUploadBaseUrl`, then install with `-f my-values.yaml`.
...
- [Auth setup](/auth-setup) — signup controls in detail
