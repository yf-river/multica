...
1. Enter your account and password.
2. The server issues a JWT cookie (browser) or exchanges the authenticated browser session for a PAT (CLI).
...
- [Self-host auth configuration](/auth-setup) — how to configure account signup controls when self-hosting
