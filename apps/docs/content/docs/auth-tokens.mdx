...
In day-to-day use of Multica you mainly deal with two kinds of credentials: browser sign-in sessions and personal access tokens. Browser sessions serve the web; personal access tokens serve the CLI, the daemon, scripts, and the API.
...
Regular Cloud and self-hosted installations never need to create them manually. The user-side CLI and daemon always use `mul_` PATs; don't construct tokens with other prefixes yourself.
