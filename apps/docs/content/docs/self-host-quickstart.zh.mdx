...
服务跑起来只是第一步。给团队日常使用或领导演示前，还要证明真实 Agent、训练评估、SOP trace、token/成本和运行证据都已经接通。
...
完整命令和证据口径见 [生产演示与观测验收](/production-observability)。
...
- [生产演示与观测验收](/production-observability) —— 用页面、API、E2E、日志和数据库证明确实可用
