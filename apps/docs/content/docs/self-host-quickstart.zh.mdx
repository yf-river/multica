...
智能体**执行**还是靠你本地跑的 [守护进程](/daemon-runtimes) + 本地装好的 [AI 编程工具](/providers)。自部署换掉的是服务器那一层，不是执行那一层。
...
如果是启动完成后的生产探针，想让数据库或 schema 异常也体现为失败，请改用 `/readyz`。

后端启动时会自动初始化空数据库。数据库非空但 schema 与当前版本不一致时，后端会拒绝启动；开发阶段不支持原地升级，需要重建数据库。
...
<Callout type="info">
**所有端口只监听 `127.0.0.1`。** `docker-compose.selfhost.yml` 把每个 publish 出来的端口都绑到 loopback —— `ss -tlnp` 不会看到 `0.0.0.0:8080`，外网/其它机器默认根本连不上。这是为了避免服务密钥和 Postgres 凭据被直接暴露到公网。要做跨机访问，请用反向代理在前面终结 TLS，详见下方 [第 5b 步 —— 跨机访问：用反向代理把服务挡在前面](#5b-跨机访问用反向代理把服务挡在前面)。
</Callout>

## 2. 首次登录并创建工作区

打开前端地址，使用账号和密码登录。如果实例允许注册，首次成功登录会创建账号；内部实例通常由管理员预置账号或邀请成员。登录后创建第一个工作区，或进入管理员已经创建好的共享工作区。

## 5. 连接命令行工具到你自己的 server

命令行装法见 [团队快速上手 → 2. 装命令行工具](/cloud-quickstart#2-装-multica-命令行工具)——Homebrew / 脚本 / PowerShell 任选。
...
流程见 [团队快速上手 → 5-6 步](/cloud-quickstart#5-创建智能体)。
...
<Callout type="info">
Usage / Runtime 看板读的是派生表 `task_usage_hourly`，由 backend 的进程内调度器周期性执行 `rollup_task_usage_hourly()` 填充。无需配置外部 cron；默认的 `pgvector/pgvector:pg17` 镜像无需改动即可工作。
</Callout>
...
这个 chart **不会模板化任何敏感值**。它通过 name 引用一个叫 `multica-secrets` 的 Secret，所以真实的 JWT、DB 和 CloudFront 密钥永远不用进 git，也不用进 `values.yaml`。先用 kubectl 一次性把命名空间和 Secret 建好：
...
默认主机名是 `multica.dev.lan`（web）和 `api.multica.dev.lan`（backend）。把它们加进 `/etc/hosts`（或者本地 DNS），指向任意一个 Ingress 可达的节点 IP 就行。要换主机名，就把 `deploy/helm/multica/values.yaml` 复制一份，改掉 `ingress.frontend.host` / `ingress.backend.host`，再把 `backend.config.appUrl` / `frontendOrigin` / `localUploadBaseUrl` 改成相应的地址，然后 `helm install ... -f my-values.yaml`。

冷集群上 backend 可能会 `Running` 但 `Not Ready` 持续几分钟，等待 Postgres 并初始化 schema——startupProbe 会兜住这一段，pod 不会被 liveness 重启。等它 `Ready` 之后：
...
完整参考——三种登录方式、同域 API/WebSocket 路由、资源限制、TLS——都在仓库的 [`SELF_HOSTING.md`](https://github.com/multica-ai/multica/blob/main/SELF_HOSTING.md#kubernetes-deployment-alternative)。

## 部署验证与运行观测

服务跑起来只是第一步。交付给团队使用前，还要证明真实 Agent、训练评估、SOP trace、token/成本和运行证据都已经接通。

最短路径：

1. 打开 **训练与评估 → 运行看板**，确认训练评估、SOP/任务观测、真实智能体准备度和验收证据不是空数据。
2. 打开 **运行历史**，展开最近一条 Agent 执行，确认能看到任务标识、模型、运行时、token、trace 和用例结果。
3. 跑 `/health`、`/readyz`、真实 Agent E2E、日志检查和数据库抽查，把结果写进验收记录。

完整命令和当前观测接口见 [部署验证与运行观测](/production-observability)。
...
- [登录与注册配置](/auth-setup) —— 注册控制详细配置
- [部署验证与运行观测](/production-observability) —— 用页面、API 和日志确认当前部署可用
- [GitHub 集成](/github-integration) —— 连接 GitHub App，同步 PR 与检查状态，并把 PR 显式关联到 issue
