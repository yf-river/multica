...
个人智能体（personal agent）只有它的所有者以及工作区 owner、admin 能分配到 issue 上。角色权限详见 [成员与权限](/members-roles)。
...
每个 issue 有一个工作区内唯一的序号，显示为 `<当前工作区前缀>-<数字>`，比如 `MUL-123`。数字在创建时由系统自动分配且不会复用；管理员修改工作区前缀后，现有 issue 的显示编号也会使用新前缀。详见 [工作区 → Issue 编号](/workspaces#issue-编号)。
