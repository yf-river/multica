...
- **工作区（workspace）** —— 工作区里任何成员都能查看和分配
- **个人（personal）** —— 只有工作区的 owner、admin，或智能体的所有者能查看和分配

新建的智能体**默认是个人的**。想让全工作区都能用，在创建时把范围选为 `workspace`，或之后在配置里改。角色权限完整对照见 [成员与权限](/members-roles)。

<Callout type="info">
个人智能体会在服务端列表和详情接口中对无权成员隐藏；它的所有者以及工作区 owner、admin 仍可管理。工作区智能体则对所有成员可见和可分配。
</Callout>
