...
1. 读取你登录时保存的凭证
2. 探测本机 `PATH` 上已安装的 AI 编程工具（内置支持 11 款：[Antigravity](/providers#antigravity)、[Claude Code](/providers#claude-code)、[Codex](/providers#codex)、[Cursor](/providers#cursor)、[Copilot](/providers#copilot)、[Gemini](/providers#gemini)、[Hermes](/providers#hermes)、[Kimi](/providers#kimi)、[Kiro CLI](/providers#kiro-cli)、[OpenCode](/providers#opencode)、[Pi](/providers#pi)）
...
<Callout type="info">
**云端运行时由团队管理员统一配置**。团队成员无需留下联系方式；如果你的工作区已启用云端运行时，可以直接选择可用环境执行智能体任务。
</Callout>
...
- **智能体层**：每个智能体默认 **20 个执行任务并发**（智能体配置里改）
