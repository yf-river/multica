...
From the web, on a remote computer, or in a headless environment, [install the Multica CLI](/cli) first, then run:
...
If the daemon cannot find a command your terminal can run, set an absolute path for the current computer:
