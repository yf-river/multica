...
**怎么知道 Autopilot 失败了**：直接打开 [Autopilots](/autopilots) 页面查看每条 Autopilot 的运行历史。单次失败不会发收件箱通知；系统只会在连续失败达到自动暂停阈值时通知负责人。
</Callout>
...
- UI 和 CLI 会为这一次操作携带稳定的请求 ID。若网络在服务端提交后中断，恢复请求返回同一条新任务，不会取消它再创建第二条任务。
...
- ✅ **真支持**——Antigravity、Claude Code、Codex、Copilot、Cursor、Hermes、Kimi、Kiro CLI、OpenCode、Pi
- ❌ **不支持**——Gemini
