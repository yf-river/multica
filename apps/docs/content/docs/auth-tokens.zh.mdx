...
| **个人访问令牌（PAT）** | 以 `mul_` 开头 | CLI / 脚本 / 直接调 API | API 创建时可选有效期；CLI 登录签发 90 天并在临近过期时续期 |
...
- **CLI**：`multica login` 会创建一个 90 天有效的 PAT；daemon 会在剩余有效期进入续期窗口时调用续期接口

直接调用 API 时可以传 `expires_in_days`。省略该字段会创建不设到期时间的 PAT；生产凭证建议明确设置有效期并定期轮换。
