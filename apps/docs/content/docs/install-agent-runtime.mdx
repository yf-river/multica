...
If the terminal finds the tool but the background daemon does not, they are usually using different `PATH` values. Restart the daemon, or set an absolute path through the matching `MULTICA_<PROVIDER>_PATH` environment variable; full configuration is in [Environment variables](/environment-variables#daemon-configuration).
...
Restart the daemon so it re-reads the environment.
