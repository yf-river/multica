...
curl -i http://<server-host>:8080/readyz  # 连同 DB + schema readiness 一起检查
...
1. **`rollup_task_usage_hourly()` 没被认领** —— Usage / Runtime 看板读的是派生表 `task_usage_hourly`，这张表必须靠 `rollup_task_usage_hourly()` 周期性填充。后端通过 DB 调度器（`sys_cron_executions`）在进程内跑 rollup；所有副本长时间下线会让这张表里没有最近的 SUCCESS 行。
...
3. **handler 被认领了、但静默报错** —— 比如 DB role / search_path 配错了。看 `sys_cron_executions` 里的 FAILED 审计行。
...
- 如果 `sys_cron_executions` 表不存在，说明数据库不是当前完整 schema；备份所需数据后重建数据库。
