...
1. **`rollup_task_usage_hourly()` 没被认领** —— Usage / Runtime 看板读的是派生表 `task_usage_hourly`，由后端进程内调度器通过 `sys_cron_executions` 周期性认领；所有副本长时间下线会让表里没有最近的 SUCCESS 行。
2. **执行失败** —— 数据库 schema 不完整、DB role 或 `search_path` 配置错误时，调度器会在 `sys_cron_executions` 写入 FAILED 审计行。
...
- 如果 `sys_cron_executions` 表不存在，重启后端让当前 migration 跑完，或手动执行 `migrate up`。
