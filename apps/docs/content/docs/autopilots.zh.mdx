...
Webhook 只接受 JSON 请求体，不接受表单编码 payload。
...
- **泄漏后立即重新生成**——在触发器上点"重新生成 URL"，或运行
  ...
  旧 URL 立即失效。
- 当前 webhook 不校验来源签名；需要强来源认证时，应在受控网关验证签名后再转发到该 bearer URL。
- 当前能查看 Autopilot 的工作区成员都能看到它的 webhook URL，因此不要把含敏感触发能力的 Autopilot 暴露给不受信任成员。
...
时 UI 会用客户端的 API origin 拼出 URL——同源 Web 没问题，
