...
前置条件：你本机已经装好至少一款受支持的 [AI 编程工具](/providers)（Claude Code、Codex 等），并跑着 [守护进程](/daemon-runtimes)。如果还没走到这一步，先看 [团队快速上手](/cloud-quickstart) 或 [自部署快速上手](/self-host-quickstart)。
...
另外 6 款（Antigravity、Hermes、Kimi、Kiro CLI、OpenCode、Pi）以及每款工具的完整能力差别（会话恢复、MCP、skill 注入路径、模型选择）见 [AI 编程工具对照](/providers)。
...
- **个人**（`personal`）—— 只有工作区的 owner、admin，或智能体的所有者能查看和使用

新建默认 `personal`。

个人智能体不会出现在无权成员的列表中。它的所有者以及工作区 owner、admin 可以查看和使用；env 值仍不会出现在普通 agent list/get 响应里。完整含义见 [智能体 → 谁能把智能体分配出去](/agents#谁能把智能体分配出去)。

## 并发上限

**并发上限**（`max_concurrent_tasks`）决定这个智能体同一时间最多同时跑几个任务，默认 **20**。达到上限的新任务留在队列排队，不会被拒绝。
