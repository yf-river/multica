...
Multica 使用账号和密码登录。是否允许新账号创建由以下变量控制：

| 环境变量 | 默认值 | 说明 |
|---|---|---|
| `ALLOW_SIGNUP` | `true` | 设为 `false` 时禁止新账号创建。内部实例建议关闭。 |
| `ALLOWED_ACCOUNTS` | 空 | 可选的账号名白名单，多个账号用英文逗号分隔。 |
...
默认不向 PostHog 上报。只有显式配置 `POSTHOG_API_KEY` 才会启用产品行为事件；`ANALYTICS_DISABLED=true` 可以强制关闭。

| 环境变量 | 默认值 | 说明 |
|---|---|---|
| `ANALYTICS_DISABLED` | `false` | 设 `true` 完全关闭后端上报 |
| `POSTHOG_API_KEY` | 空 | PostHog project API key；空值使用 no-op client，不发送事件 |
