...
在 Multica 里，一个**运行时**（runtime）就是你机器上的守护进程，配上守护进程在 `PATH` 里扫到的某一款 AI 编程工具。如果「运行时」页面显示 **未检测到支持的工具**，说明守护进程扫了 `PATH`，但 12 款它认得的工具一个都没找到。装下面任意一款（或几款），回到该页面重新扫描，几秒内运行时就会出现。
...
4. **回到「运行时」页面。** 页面会自动刷新，几秒内就能看到新运行时。
