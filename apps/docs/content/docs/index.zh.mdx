...
- **本地 [守护进程](/daemon-runtimes)** — 你在自己的机器上运行 `multica daemon`，由它调用本地安装的 [AI 编程工具](/providers)。目前内置 11 款：[Antigravity](/providers#antigravity)、[Claude Code](/providers#claude-code)、[Codex](/providers#codex)、[Cursor](/providers#cursor)、[Copilot](/providers#copilot)、[Gemini](/providers#gemini)、[Hermes](/providers#hermes)、[Kimi](/providers#kimi)、[Kiro CLI](/providers#kiro-cli)、[OpenCode](/providers#opencode)、[Pi](/providers#pi)。你的 API 密钥、工具链、代码目录都保留在本地。

<Callout type="info">
**云端运行时由团队管理员统一配置**。团队成员无需留下联系方式；如果你的工作区已启用云端运行时，可以直接在运行时页面选择可用环境。
</Callout>
...
<NumberedCards>
  <NumberedCard number="01" title="团队内部环境" href="/cloud-quickstart" tag="团队配置">
    连接管理员已经部署好的 Multica 服务。安装命令行工具并在本地运行守护进程，约 5 分钟完成。
  </NumberedCard>
  ...
  <NumberedCard number="03" title="桌面应用" href="/desktop-app" tag="推荐">
    原生多标签窗口。内置命令行工具并在启动时自动拉起守护进程——安装后无需运行任何命令即可使用。可连接团队内部或自部署后端。
  </NumberedCard>
</NumberedCards>
