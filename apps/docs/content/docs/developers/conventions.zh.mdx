...
| `apps/mobile/` | `@multica/core` 的类型和纯函数 | React 页面、store 和平台实现 |

多个客户端都需要的逻辑，**必须**抽到共享包。"小段重复"也不算例外。Mobile 拥有独立的 UI、数据层和发布流程，只共享类型与纯函数。
...
- 客户端可能连接更新的 backend，不能假设前后端永远同版本
...
- Web-only：放 `web` 段
- Mobile-only：放 `mobile` 段

参考 `auth.json` 中的客户端专属文案示例。
