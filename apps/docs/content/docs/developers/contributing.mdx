...
`make up` treats an environment as a named object: it allocates the API and Web ports,
...
removes the database, CLI profile, daemon workspaces and
...
Place shared features according to responsibility:
...
4. Keep Next.js and routing adapters in the corresponding app.
5. Wire shared pages into Web and reuse only platform-independent types and functions in Mobile.
...
| Web or Mobile platform wiring | Corresponding `apps/*` directory |
