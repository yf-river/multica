...
Multica 由一个 Go backend、Web 和移动客户端以及运行在执行电脑上的守护进程组成。PostgreSQL 保存协作数据；守护进程领取 task，并调用本机的 AI 编程工具。
...
| `packages/views/` | Web 业务页面和组件 | React |
...
路由、cookie 等平台能力留在应用层。共享页面通过 `NavigationAdapter` 导航，不直接导入 `next/*` 或 `react-router-dom`。
...
API 返回值在 `packages/core/api/` 边界通过 zod schema 解析。客户端可能连接更新的 backend，因此不能直接把网络 JSON 强制转换成 TypeScript 类型。
