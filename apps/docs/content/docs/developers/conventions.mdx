...
| `apps/mobile/` | types and pure functions from `@multica/core` | React pages, stores, and platform implementations |

If logic appears in multiple clients, it MUST be extracted to a shared package. There are no exceptions for "small" duplication. Mobile owns a separate UI, data layer, and release process; it shares only types and pure functions.
...
- Clients may connect to a newer backend; never assume that frontend and backend versions always match.
...
- Mobile-only: `mobile` section

See `auth.json` for the canonical example of client-specific copy.
