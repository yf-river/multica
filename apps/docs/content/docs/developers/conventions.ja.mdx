...
| `apps/mobile/` | `@multica/core` の型と純粋関数 | React ページ、store、プラットフォーム実装 |

複数のクライアントに同じロジックが現れる場合は、必ず共有パッケージに抽出しなければなりません。「ささいな」重複という例外はありません。Mobile は独自の UI、データ層、リリースフローを持ち、型と純粋関数だけを共有します。
...
- クライアントがより新しい backend に接続する場合があるため、フロントエンドとバックエンドが常に同じバージョンだと仮定しない
...
クライアント専用コピーの例は `auth.json` を参照してください。
