...
- **Web**: connect this computer, or another computer, from the command line in the next step.
...
From the web, click **Add a computer** in the top right, then run the two commands from the dialog in a terminal on the target computer:
...
| Symptom | Check first |
|---|---|
| No runtime found | Confirm the AI coding tool is installed and runs in a terminal, then rerun `multica setup`. |
| Runtime shows offline | Run `multica daemon status`, and `multica daemon start` if it is not running. |
