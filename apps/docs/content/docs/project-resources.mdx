...
The connected daemon adds a local directory for the project; browsers do not read folders on your computer directly.
...
1. Make sure the local daemon is online.
...
A local directory has two execution modes, set per resource with `execution_mode`. The UI labels them **Direct** and **Parallel**; the API, the CLI and the rest of this page use the identifiers.
...
Because the worktree lives in the runtime's workspace directory, it is reclaimed on the normal cleanup schedule. Two things persist in your repo: the branch, and one hidden ref per branch under `refs/multica/local-state/` recording who owns it and what your directory looked like when it was last continued. The hidden refs never appear in `git branch`; list them with `git for-each-ref refs/multica`. Multica deletes each one with its branch, and drops any whose branch you deleted yourself the next time a task runs on that repo — or you can remove them at once with `git for-each-ref --format='%(refname)' refs/multica | xargs -n1 git update-ref -d`.
